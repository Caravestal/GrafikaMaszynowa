...
class Opencv(BaseImage):
    ...
    def adaptacyjne(self) -> BaseImage:
        lena_gray = cv2.imread(self.path, cv2.IMREAD_GRAYSCALE)
    ...
    def detekcjaCanny(self) -> BaseImage:
        lena_gray = cv2.imread(self.path, cv2.IMREAD_GRAYSCALE)
    ...
    def wyrownywanieCLAHE(self) -> BaseImage:
    ...
    def dtekcjaProstych(self) -> BaseImage:
    ...
    def krawedzieLini(self) -> BaseImage:
        lines_img = cv2.imread(self.path, cv2.IMREAD_GRAYSCALE)
        _, lines_thresh = cv2.threshold(
            lines_img,
            thresh=0,
